import { GlassPanel, Button } from "../../shared/ui";

export default function Products() {
  return (
    <section id="products" className="py-16 md:py-24 bg-white/50">
      <div className="max-w-6xl mx-auto px-6">
        <h2 className="text-2xl md:text-3xl font-semibold mb-8 md:mb-12">Products</h2>

        <GlassPanel className="w-full md:max-w-xl p-6 md:p-8">
          <h3 className="text-xl md:text-2xl font-medium">AI Interview Platform</h3>

          <p className="mt-3 text-muted leading-relaxed text-sm md:text-base">
            A focused interview environment designed for fairness, consistency, and clarity.
          </p>

          <div className="mt-6">
            <Button variant="ghost" className="pl-0 text-accent font-semibold flex items-center gap-2">
              Open app <span className="text-lg">→</span>
            </Button>
          </div>
        </GlassPanel>
      </div>
    </section>
  );
}
